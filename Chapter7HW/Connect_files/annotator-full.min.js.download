...
(function() {
  ...
  function getNodesInRange(range) {
      ...
      var end = range.end;
      var commonAncestor = range.commonAncestor;
      var nodes = [];
      var node;
  ...
  getCurrentScaleValue = function() {
    /*var topDiv = $('div.annotator-wrapper');
    if (topDiv !== undefined) {
      var transformValue = topDiv.css("transform");
      var matrixRegex = /matrix\((-?\d*\.?\d+),\s*0,\s*0,\s*(-?\d*\.?\d+),\s*0,\s*0\)/;
      var matches = transformValue.match(matrixRegex);
      if(typeof matches !== 'undefined' && matches != null && matches.length > 1) {
        var scaleValue = parseFloat(matches[1]);
        return scaleValue;
      } else if (!isFirefox) {
        var zoomValue = topDiv.css("zoom");
        var scaleValue = 1;
        if(zoomValue.indexOf("%") !== -1)
          scaleValue = parseFloat(zoomValue.substring(0, zoomValue.length-1)) / 100;
        else scaleValue = parseFloat(zoomValue);
        return scaleValue;
      };
    }*/
    return 1;
  ...
  Util.mousePosition = function(e, offsetEl) {
    var offset;
    var cScale = getCurrentScaleValue();
    offset = $(offsetEl).position();
    // make sure that nothing is placed outside of the view area. LSTRE-5674
    var leftPos =  Math.round(e.pageX / cScale - offset.left + 25); // 25 - body's margin
    return {
      top: Math.round(e.pageY / cScale - 10), // 10 - just for better looking
      left: (leftPos < 25) ? 25 : leftPos // 25 - just for better looking
    };
  };
  ...
  Annotator = (function(_super) {
    ...
    Annotator.prototype.events = {
      ...
      ".annotator-create li click": "onAdderMousedown",
    ...
    Annotator.prototype._setupAdder = function() {
      this.adder = new Annotator.Adder();
      this.adder.hide();
      this.adder.element.appendTo(this.wrapper);
      return this;
    }
    ...
    Annotator.prototype._setupViewer = function() {
      ...
      this.viewer.hide().on("edit", this.onEditAnnotation).on("delete", this.onDeleteAnnotation).addField({
        load: function(field, annotation) {
          ...
          var isTouch = Annotator.Plugin.Touch.isTouchDevice() || getUrlParameter("annotatorforce") == "true";
          if(isTouch) jQuery(".annotator-annotation").data('annotation', annotation);
    ...
    Annotator.prototype.getSelectedRanges = function() {
      ...
      ranges = [];
      rangesToIgnore = [];
      // added additional check for focusOffset to fix LSTRE-3768
    ...
    Annotator.prototype.showEditor = function(annotation, location) {
      ...
      this.editor.load(annotation).show();
    ...
    Annotator.prototype.showViewer = function(annotations, location) {
      if(/iPad|iPhone|iPod/.test(navigator.platform))
        this.viewer.element.css("-webkit-transform", "translate3d(" + location.left + "px," + location.top + "px,0px)");
      else this.viewer.element.css(location);
    ...
    Annotator.prototype.checkForEndSelection = function(event) {
      ...
      // don't display annotator adder if annotator editor is displayed.
      if ($('form.annotator-widget').is(':visible')) {
        return;
      }
      //LSTRE-16
      /*if ($(event.target).closest(".annotator-widget").length > 0)
        return; */

      // this one is the fix for LSTRE-3768. Along with additional check on linenumber 1299
      // check that selected text is received.
      ...
      for (_k = 0, _len2 = _ref1.length; _k < _len2; _k++) {
        ...
        for(i = 0; i < nodes.length; i++)
          if ($(nodes[i]).hasClass('annotator-hl')) {
            this.adder.hide();
            return;
          }
      ...
      if (event && this.selectedRanges.length) {
        if (appParameters["isReadexMode"] && window.getSelection().toString().length > 1000) {
          $(".annotator-warning-sizelimit").css(Util.mousePosition(event, this.wrapper[0])).removeClass("annotator-hide");
          return this.adder.hide();
        }
        else {
          $(".annotator-warning-sizelimit").addClass("annotator-hide");
          return this.adder.css(Util.mousePosition(event, this.wrapper[0])).show(); // LSTRE-16 off
        }
        //LSTRE-16
        /*this.adder.css(Util.mousePosition(event, this.wrapper[0])).show();
        this.ignoreMouseup = true;
        $("#annotator-is_underline").prop("checked", true);
        $("#annotator_stickie").click();
        this.ignoreMouseup = false;
        return this; */
      } else {
        $(".annotator-warning-sizelimit").addClass("annotator-hide");
        return this.adder.hide(); //LSTRE-16 off
        //LSTRE-16: return this.editor.hide();
      }
    };
    ...
    Annotator.prototype.onAdderClick = function(event) {
      ...
      position = this.adder.element.position();
      var isTouch = Annotator.Plugin.Touch.isTouchDevice() || getUrlParameter("annotatorforce") == "true";
      var cScale = 1;
      if(isFirefox || isIE) cScale = getCurrentScaleValue();
      position.left /= cScale;
      position.top /= cScale;
      this.adder.hide();
      if (!isTouch)
        $(".annotator-warning-sizelimit").addClass("annotator-hide");
      var annotation = this.editor.annotation;
      if (!this.isCreatingAnnotation) {
        ...
        cleanup = function() {
          _this.isCreatingAnnotation = false;
      ...
      if (this.isCreatingAnnotation) {
        if (arguments[1] == "star") {
          if (annotation.is_important === 1)
            annotation.is_important = 0;
          else annotation.is_important = 1;
        }
        else if (arguments[1] == "dont-get")
          if (annotation.is_not_understood === 1)
            annotation.is_not_understood = 0;
          else annotation.is_not_understood = 1;
      }
      else {
        if (arguments[1] == "star")
          annotation.is_important = 1;
        else if (arguments[1] == "dont-get")
          annotation.is_not_understood = 1;
      };
      
      if (annotation.is_important) {
        this.element.find('.annotator-star-add img').attr("src", Annotator.prototype.imgStarOn);
        this.element.find('.annotator-star-add').css({color:"#F96464"});
      }
      else {
        this.element.find('.annotator-star-add img').attr("src", Annotator.prototype.imgStarOff);
        this.element.find('.annotator-star-add').css({color:"#3F3F3F"});
      }
      if (annotation.is_not_understood) {
        this.element.find('.annotator-dont-getit-add img').attr("src", Annotator.prototype.imgDontGetOn);
        this.element.find('.annotator-dont-getit-add').css({color:"#F96464"});
      }
      else {
        this.element.find('.annotator-dont-getit-add img').attr("src", Annotator.prototype.imgDontGetOff);
        this.element.find('.annotator-dont-getit-add').css({color:"#3F3F3F"});
      };
      if ((arguments[1] == "star" || arguments[1] == "dont-get") && isTouch) {
        if (!this.isCreatingAnnotation) {
        ...
        // this one is the fix for LSTRE-3768. Along with additional check on linenumber 922
        // If it fail to save a selected line then do not display note editor
        if (annotation.ranges.length == 0) {
          this.isCreatingAnnotation = false;
          return;
        }
        if (isTouch && !this.isCreatingAnnotation)
          this.isCreatingAnnotation = true;
  ...
  if (((_ref1 = g.document) != null ? _ref1.evaluate : void 0) == null) {
    $.getScript(sbScriptLocation + 'vendor/xpath.min.js');
  }

  if (g.getSelection == null) {
    $.getScript(sbScriptLocation + 'vendor/ierange.min.js');
  }

  if (g.JSON == null) {
    $.getScript(sbScriptLocation + 'vendor/json2.min.js');
  }
  ...
  Annotator.Widget = (function(_super) {
    ...
    Widget.prototype.checkOrientation = function() {
      ...
      current = {
        top: rect.top * cZoom,
        right: (rect.left + widget.width() * cScale) * cZoom
      };
      if ((current.top - viewport.top) < 0) {
        this.invertY();
      }
      if ((current.right - viewport.right) > -110) {
        this.invertX();
      }
      return this;
    };
  ...
  function makeLI(id, caption, image) {
    var a = $("<a>")
      .attr("id", id)
      .attr("href", "#")
      .append(makeIMG(image));
  ...
  Annotator.Adder = (function(_super) {
    __extends(Adder, _super);

    function Adder(options) {
      this.hide = __bind(this.hide, this);
      this.show = __bind(this.show, this);
      Adder.__super__.constructor.call(this, $(this.html)[0], options);
    }

    Adder.prototype.html = 
      $("<div>")
        .append($("<div>")
          .addClass("annotator-outer")
          .append($("<ul>")
            .addClass("annotator-widget annotator-create")
            .append(makeLI("annotator_bookmark", "Bookmark", fixScheme(sbScriptLocation + "images/smartbook/mobilereader/icon/star.png")))
            .append(makeLI("annotator_dontgetit", "I don\'t get it", fixScheme(sbScriptLocation + "images/smartbook/mobilereader/icon/question.png")))
            .append(makeLI("annotator_stickie", "Note", fixScheme(sbScriptLocation + "images/smartbook/mobilereader/icon/note.png")))
            .append(makeLI("annotator_cancel", "", fixScheme(sbScriptLocation + "images/smartbook/mobilereader/icon/x.png")))
          )
        )
      .html();

    Adder.prototype.css = function(style) {
      this.element.css(style);
      return this;
    };

    Adder.prototype.hide = function(event) {
      Annotator.Util.preventEventDefault(event);
      this.element.addClass(this.classes.hide);
      return this.publish('hide');
    };

    Adder.prototype.show = function(event) {
      Annotator.Util.preventEventDefault(event);
  ...
  Annotator.Editor = (function(_super) {
    ...
    //   $("<div>")
    //     .addClass("annotator-outer annotator-editor")
    //     .append($("<form>")
    //       .addClass("annotator-widget")
    //       .append($("<ul>")
    //         .addClass("annotator-widget")
    //       )
    //       .append($("<div>")
    //         .addClass("annotator-controls")
    //         .append(makeLI("annotator_bookmark", "Bookmark", fixScheme(sbScriptLocation + "images/smartbook/mobilereader/icon/star.png")))
    //         .append(makeLI("annotator_dontgetit", "I don\'t get it", fixScheme(sbScriptLocation + "images/smartbook/mobilereader/icon/question.png")))
    //       )
    //     );
    ...
    "<a href=\"#save\" class=\"annotator-save annotator-focus\"><div style=\"height: 35px;\"></div>" + _t('Save') + "</a>"+
    "<a href=\"#cancel\" class=\"annotator-cancel\"><div style=\"height: 35px;\"></div>" + _t('Cancel') + "</a>"+
    "</div>"+
    "</form>"+
    "</div>";
    ...
    function Editor(options) {
      ...
      this.fields = [];
      this.annotation = {};
      this.is_important_tmp = 0;
      this.is_not_understood_tmp = 0;
    }
    ...
    Editor.prototype.load = function(annotation) {
      ...
      if (annotation.is_important) {
        this.element.find('.annotator-starred-save img').attr("src", Annotator.prototype.imgStarOn);
        this.element.find('.annotator-starred-save').css({color:"#F96464"});
      }
      else {
        this.element.find('.annotator-starred-save img').attr("src", Annotator.prototype.imgStarOff);
        this.element.find('.annotator-starred-save').css({color:"#3F3F3F"});
      }
      if (annotation.is_not_understood) {
        this.element.find('.annotator-dont-get-save img').attr("src", Annotator.prototype.imgDontGetOn);
        this.element.find('.annotator-dont-get-save').css({color:"#F96464"});
      }
      else {
        this.element.find('.annotator-dont-get-save img').attr("src", Annotator.prototype.imgDontGetOff);
        this.element.find('.annotator-dont-get-save').css({color:"#3F3F3F"});
      }
      this.annotation = annotation;
      this.is_important_tmp = annotation.is_important;
      this.is_not_understood_tmp = annotation.is_not_understood;
      ...
      $('label[for="annotator-is_important"]')
        .html("")
        .append(makeIMG(fixScheme(sbScriptLocation + "images/smartbook/mobilereader/icon/star.png")))
        .append("<br />Bookmark");

      $('label[for="annotator-is_not_understood"]')
        .html("")
        .append(makeIMG(fixScheme(sbScriptLocation + "images/smartbook/mobilereader/icon/question.png")))
        .append("<br />I don\'t get it");
  ...
  Annotator.Viewer = (function(_super) {
    ...
    function Viewer(options) {
      ...
      var isTouch = Annotator.Plugin.Touch.isTouchDevice() || getUrlParameter("annotatorforce") == "true";
      if (isTouch) {
        this.element.html(//'<div class="annotator-outer annotator-viewer annotator-touch-widget annotator-touch-viewer noselect">' +
          '<ul class="annotator-touch-viewer-menu annotator-widget annotator-listing annotator-annotation">' +
          '<li class="annotator-item"><img id="star-item" /><img id="dont-get-item" style="left: 15%" /><div><i>No comment</i></div></li>' +
          '<li class="annotator-edit"><img src="' + fixScheme(sbScriptLocation + "images/smartbook/mobilereader/icon/note_.png") + '"/></li>' +
          '<li class="annotator-delete"><img src="' + fixScheme(sbScriptLocation + "images/smartbook/mobilereader/icon/trash.png") + '"/></li></ul>');
      }
    }
    ...
    Viewer.prototype.show = function(event) {
      ...
      else this.element.removeClass(this.classes.hide);
    ...
    Viewer.prototype.isShown = function() {
      ...
      else return !this.element.hasClass(this.classes.hide);
    };
    ...
    Viewer.prototype.hide = function(event) {
      ...
      if(isTouch) {
        ...
        jQuery("img#star-item").css("visibility", "hidden");
        jQuery("img#dont-get-item").css("visibility", "hidden");
      }
      else
        this.element.addClass(this.classes.hide);
      return this.publish('hide');
    ...
    Viewer.prototype.load = function(annotations) {
      ...
      var isTouch = Annotator.Plugin.Touch.isTouchDevice() || getUrlParameter("annotatorforce") == "true";
      if(isTouch) { //annotations.length and fields.length supposed to be 1
        var field2 = jQuery("ul.annotator-touch-viewer-menu li.annotator-item div");
        annotation = this.annotations[0];
        if (annotation.text) {
            $(field2).html(Util.escape(annotation.text));
          } else {
            $(field2).html("<i>" + (_t('No comment')) + "</i>");
          };
        field = this.fields[0];
        field.load(field2, annotation);
      }
      else {
